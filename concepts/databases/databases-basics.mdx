...
![Create](/images/databases/1/create.gif)

## Configuring a database

**After creating a database, the next step is to configure it.** 1. Click on the **configuration** header to expand it.
...
![Databases Config](/images/databases-config.png)
...
![Dimensions Pills](/images/dimensions-pills.png)

**Tip #1**– To get a quick look at the structure of your data source, click the **table icon** to preview sample data.

![Databases Config Preview](/images/databases-config-preview.png)

**Tip #2**– If you want to limit the database to a specific subset of data from the source, apply **afilter** within the database configuration.

![Databases Config Filters](/images/databases-config-filters.png)
...
3. **Dimension**– A predefined list of values to help categorize your data. Unlike drivers, dimensions do not change over time. For example, in a GL database, dimensions could include **Vendor, Location, or Department**. Learn more about [dimensions here](https://docs.runway.com/concepts/dimensions/dimensions-basics).
...
![Databases New Column](/images/databases-new-column@2x.png)
...
![Show Column As Timeseries](/images/databases/1/show-column-as-timeseries.gif)
...
![Databases Hide Column](/images/databases-hide-column.png)
...
![Show All As Timeseries](/images/databases/1/show-all-as-timeseries.gif)

When you switch to this view, the database orientation changes. What was previously a **row** in the database becomes **asection**, while dimensions and drivers that were displayed as columns are now shown as **rows**. This allows you to see time-series data for all drivers at once.

![Database As Timeseries](/images/database-as-timeseries.png)
...
![Database Hide In Block](/images/database-hide-in-block.png)
...
If you need to manually add a row to a database—for example, a new **vendor** in an **Income Statement by Vendor** database or a new **assumption** in an **assumptions** database—click **+ Add item.** After adding the row, set values for the **dimensions** used in the **Segment by** for this database.

![Database Add Row](/images/database-add-row.png)

**Important:** Segmentation determines the **granularity** of your database. Each row should have **aunique combination** of dimension values.

- If a row does **not** have a unique combination, you’ll see **awarning** next to it.
...
<Note>
  For HRIS databases, the process of adding a row is different. Please follow the steps [here](/concepts/databases/hris-databases).
</Note>
...
![Database Remove From Config](/images/database-remove-from-config.png)
...
![Database Delete Property](/images/database-delete-property.png)
