...
In typical Runway databases, [**segmentation determines the granularity of your database**](https://docs.runway.com/concepts/databases/databases-basics#adding-a-row). Each row has **aunique combination** of dimension values.

In HRIS databases, each row represents a single **employee, identified by their name.**

![Hris Rows](/images/databases/hris/hris-rows.png)
...
2. In the formula editor, search for and reference the field column from the Employees database. You can search by the field column name, but not by the employee name or any of its attributes.
   ...
   ![Search For Field In Formula](/images/databases/hris/search-for-field-in-formula.png)
3. Add an aggregator (e.g. sum) and use filters to define which employees to include.

   ![Fields Filters Informula](/images/databases/hris/fields-filters-informula.png)
...
- Each row inherits the default formula set for field columns, and **these formulas cannot be overridden for individual rows.** - When actual employee data is later imported from your HRIS system, **it won't automatically reconcile with manually added placeholder employees**. You'll need to manage this reconciliation.
...
![Hris Columns](/images/databases/hris/hris-columns.png)
...
![Hris Column Formulas](/images/databases/hris/hris-column-formulas.png)
