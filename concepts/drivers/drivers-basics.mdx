...
<Info>
  This article focuses on the behavior of drivers in driver tables. Drivers can also be used in databases, which have some additional behavior you can read about [here](/concepts/formulas/formulas-basics).
</Info>

## What is a Driver?

A **Driver** in Runway is a structured chunk of **timeseries data**. It provides clarity with **ahuman-readable name**, representing the values that drive your business model—key metrics that shape your bottom line.
...
- **Dimensional**– Drivers can have **dimensions** that structure your model as complexity grows. Learn more in [Dimensions basics](/concepts/dimensions/dimensions-basics).

![Drivers](/images/drivers/basics/drivers.png)
...
![Forecast Actuals](/images/drivers/basics/forecast-actuals.png)
...
![Formulas Default](/images/drivers/basics/formulas-default.png)
...
Drivers store **timeseries data** in one of three formats:
...
- **Attributes** – Used for dimension values, enabling drivers to store categorical data from dimensions. This is particularly useful when working with HRIS data or other sources where you need to track dimension-based attributes over time.

<Note>
  When entering explicit dates in formulas, use the format 'YYYY-MM-DD' (enclosed in single quotes).
</Note>

![Driver Types](/images/drivers/basics/driver-types.png)
...
We tend to find that Date drivers are most useful when working with [drivers in databases](/concepts/databases/formulas-in-databases).
