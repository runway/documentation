...
<Info>
  This article focuses on the behavior of drivers in driver tables. Drivers can
  also be used in databases, which have some additional behavior you can read
  about here. (LINK: coming soon)
</Info>
...
- **Dimensional** – Drivers can have **dimensions** that structure your model as complexity grows. More on this in \[LINK: article on dimensions + drivers (coming soon)\].
...
<Note>
  When entering explicit dates in formulas, use the format 'YYYY-MM-DD'
...
<Warning>
  If these constraints aren’t met, the driver’s values will display a **TYPE
  error**, indicating a mismatch between the formula output and driver type.
</Warning>
