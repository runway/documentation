...
![Model Groups](/images/models/model-groups.png)

In addition to improving organization and readability, groups also make it easier to find and reuse drivers. When adding drivers to a block, you can **search by group name** and include all or a subset of drivers from that group. ![Model Groups In Search](/images/models/model-groups-in-search.png)

You can also **reference an entire driver group in formulas**, instead of selecting each driver individually.

![Model Groups In Formula](/images/models/model-groups-in-formula.png)

## How to group drivers?

There are a couple of ways to create groups in a model:

1. **Select and group:** Select the drivers you'd like to group, then right-click and choose **Group** — or press `Cmd` / `Ctrl` + `G`. A new group will be created with the selected drivers. Give the group a descriptive name.
   ![Model Grouping 1](/images/models/model-grouping-1.gif)
2. **Create group first:**\
   ...
   ![Model Grouping 2](/images/models/model-grouping-2.png)
