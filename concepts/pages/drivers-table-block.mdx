...
3. Choose the **drivers** you’d like to include in the table. You can **add or remove drivers** later, not just when first creating the block.
...
3. Choose which columns to show or hide:
   - **Data** - **Actual Formula** - **Forecast Formula** - **Used By** - **Graph** - **Rollup**

![Properties](/images/pages/driver-table/properties.png)
...
2. If you **right-click** on a driver name or click the **6-dot menu** to the left of any driver name, you can select **+ Insert new driver above** or **+ Insert new driver below.**
   ...
   ![Adddriveraboveorbelow](/images/drivers/basics/adddriveraboveorbelow.png)
...
<Check>
  Removing a driver from the block **does not delete it from Runway** — it only removes it from this specific table. See [this article](https://docs.runway.com/concepts/drivers/duplicate-remove-delete) for details on removing vs. deleting.
</Check>

![Remove Delete Driver](/images/pages/driver-table/remove-delete-driver.png)
...
2. Choose a rollup option. You can pick from:
   - **Monthly**, **Quarterly**, or **Annual** - **Monthly & Quarterly**, **Monthly & Annually**, or **Monthly, Quarterly & Annually** - For additional customization options, click **Custom rollup**, which provides additional options such as:
     - **Quarter to date**, **Half-year to date**, or **Year to date** (includes the current calendar month)
     - **Quarter to last close**, **Half-year to last close**, or **Year to last close** - **Trailing 3, 6, or 12 months** (relative to the current calendar month)

![Time Period Rollup](/images/pages/driver-table/time-period-rollup.png)

### Selecting the aggregation

For each driver, you can define **how its values are aggregated** when using a coarser time period: **Right-click** the driver name → Select **Roll up by** → Choose from: **Sum** (default), **Average**, **First**, or **Last.**

![Time Period Aggregation](/images/pages/driver-table/time-period-aggregation.png)

You can also show the **Rollup** property as a column:

1. Click **Customize → Properties**, then enable **Rollup.** 2. In the table, click the **Rollup cell** for a driver to adjust its aggregation method directly from there.

![Rollup Property](/images/pages/driver-table/rollup-property.png)
...
3. Pick which metrics to display:
   - **Value** - **Variance** - **Variance %** 4. Each comparison will appear as **anew row** in your driver table, just below the original driver.

![Compare Time Period](/images/pages/driver-table/compare-time-period.png)
