...
<Info>
  **Which approach should I use?**
  ...
  - **New HRIS databases** - Function like regular databases and use database blocks with segmentation (recommended for new implementations)
  - **Legacy HRIS databases** - Use the "Expand by Dimension" feature (still supported for existing models)
  
  If you're unsure which type of HRIS database you have, check with your Runway team or try the new approach first—it will work with both types but provides better flexibility.
</Info>
...
<Tabs>
  <Tab title="New HRIS databases">
    ### Database block with segmentation (recommended)

    New HRIS databases function like regular databases in Runway, which means you can reference them directly in formulas. When segmenting by dimensions that change over time (like Department or Job Title), follow this pattern to ensure historical changes are captured correctly.

    1. **Create a database block on your page**

       Add a database block and point it to your Employees database (or the HRIS database you're using).

    2. **Segment by the dimension in the query**

       In the database block settings, segment by the dimension you want to track (e.g., Department). This pulls the dimension values directly from the query, ensuring any new departments or changes are automatically captured.

       <Info>
         Segmenting in the query (rather than using the dimension as a filter in formulas) ensures that the time series underneath the dimension is properly utilized. This is critical for capturing historical changes like when an employee moves from one department to another.
       </Info>

    3. **Create column formulas that reference the database**

       Create column formulas directly in the database block. For example, to create a Headcount column:
       
       - Add a new column called "Headcount"
       ...
       Because you've segmented by Department in the query and are using Department as a dimension driver in the formula, Runway will properly match the time series and capture changes over time.
    ...
    Once you've set up your database block with segmentation, skip to [Step 4: Add new employees](#step-4-add-new-employees) below.
  </Tab>
  ...
  <Tab title="Legacy HRIS databases">
    ...
    After expanding by dimension, continue to [Step 3: Build the formulas](#step-3-build-the-formulas) below.
  </Tab>
</Tabs>
...
<Note>
  **This step is only required for legacy HRIS databases** using the "Expand by Dimension" approach. If you're using the new database block approach, skip to [Step 4](#step-4-add-new-employees).
</Note>
