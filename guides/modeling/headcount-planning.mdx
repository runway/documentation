...
HRIS databases now function like regular databases in Runway, which means you can reference them directly in formulas. However, when segmenting by dimensions that change over time (like Department or Job Title), you'll need to use a specific approach to ensure historical changes are captured correctly.

### Recommended approach: Database block with segmentation

When building departmental headcount models or segmenting by any dimension that can change over time, follow this pattern:
...
3. **Create column formulas that reference the database**
   ...
   Instead of using "Expand by Dimension" (the legacy approach), create column formulas directly in the database block. For example, to create a Headcount column:
...
### Legacy approach: Expand by dimension

<Warning>
  The **Expand by Dimension** feature is a legacy approach that should be avoided for new implementations. It does not automatically create drivers when new attributes are added to a dimension, making it unsuitable for dynamic dimensions.
  
  Use the database block segmentation approach described above instead.
</Warning>

If you're working with an older headcount model that uses Expand by Dimension:
