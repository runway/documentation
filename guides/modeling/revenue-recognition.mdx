...
4. Use DateDiff() function to calculate a Contract Term Length based on contract start date and Contract End Date.
   ...
   ![CleanShot 2025-05-14 at 14.19.09@2x (2).png](/images/CleanShot2025-05-14at14.19.09@2x\(2\).png)
...
7. Click \+ Add Rule to add filters to the formula. These will make sure the contract value hits your model only between Contract start and End dates.
   ...
   - `Segment is This Segment` will ensure this logic gets applied across all segments of the driver, which in this example, means across all slices of Contract ID x Customer Name. You can review how to create dynamic formulas using _This Segment_ [here](https://docs.runway.com/concepts/databases/formulas-in-databases#example%3A-dynamic-default-formula-using-this-segment)

![Clean Shot2025 05 14at21 07 41@2x Pn](/images/CleanShot2025-05-14at21.07.41@2x.png)

8. Finally, divide by Contract Term Length (Months) to calculate a monthly value. Each driver will be populates from Contract Start to Contract End date
...
## Step 4: Wire Contracts database into Revenue forecast driver

Finally, you will need to wire up the Contract database to drive Revenue forecasts upstream in your model.

1. Either navigate to the Revenue driver if it already exists in your Runway instance or create a new driver in a Model or on a Page
2. Click into the forecast formula and create a sum formula
   - You can create a sum using a driver reference by directly referencing Monthly Recognized Revenue All Customer Names All Contract IDs
   ...
   You may also create a sum over the database column reference which will allow you to apply filters based on non-Segment dimensions if necessary
...
![CleanShot 2025-05-14 at 21.28.36@2x.png](/images/CleanShot2025-05-14at21.28.36@2x.png)