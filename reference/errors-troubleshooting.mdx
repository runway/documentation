...
Admins may see these surfaced in the sidebar Errors panel. The panel is not available to every user, so the important action is to fix the dependency cycle itself.
